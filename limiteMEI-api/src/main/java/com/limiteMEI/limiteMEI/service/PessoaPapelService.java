...
import com.limiteMEI.limiteMEI.domain.*; import com.limiteMEI.limiteMEI.dto.pessoa.PessoaDTO; import com.limiteMEI.limiteMEI.dto.relatorio.RelatorioPessoaFiltroDTO; import com.limiteMEI.limiteMEI.enums.PapelPessoaEnum; import com.limiteMEI.limiteMEI.mapper.PessoaMapper; import com.limiteMEI.limiteMEI.repository.PessoaPapelRepository; import org.springframework.stereotype.Service; import java.util.*;
...
public class PessoaPapelService{
 ...
 public List<PessoaDTO> relatorio(PapelPessoaEnum papel,RelatorioPessoaFiltroDTO filtro){String nomeFiltro=filtro.getNome()==null||filtro.getNome().isBlank()?null:filtro.getNome().trim();String documentoFiltro=filtro.getDocumento()==null||filtro.getDocumento().isBlank()?null:filtro.getDocumento().trim();return repository.relatorioPessoas(empresaAtual.get().getId(),papel,nomeFiltro,documentoFiltro,filtro.getTipoPessoa()).stream().map(PessoaPapel::getPessoa).map(mapper::toDTO).toList();}
